...
function delete_user($id){
  ...
  function cari($keyword) {
    $conn = koneksi();
    $query = "SELECT * FROM tampilan NATURAL JOIN kategori
...
if(!$gambar){
  ...
  $query = "INSERT INTO
              tampilan
            VALUES
            (NULL, '$gambar', '$judul',  '$harga', '$kategori');
          ";
  mysqli_query($conn, $query);
  echo mysqli_error($conn);
  return mysqli_affected_rows($conn);
...
{
  $conn = koneksi();
  mysqli_query($conn, "DELETE FROM tampilan WHERE id= $id") or die (mysqli_error($conn));
  return mysqli_affected_rows($conn);
}
